...
- Si le candidat répond par un seul mot ou une phrase très courte et évasive, sans développement : attribue directement 1⭐, explique qu'une réponse aussi minimale ne convaincrait aucun recruteur en situation réelle, puis propose une structure claire à suivre (ex: 1. une affirmation directe, 2. un exemple concret vécu, 3. le résultat obtenu, 4. le lien avec le poste visé) et repose la MÊME question — n'avance jamais à la suivante sur une réponse aussi faible.
...
function Landing({ onStart, candidate, onFindProfile, onLogout, onSessionInvalid, sessionMessage, onDismissSessionMessage }) {
  ...
  async function handleLookup(e) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.found) {
        onFindProfile({ fullName: data.fullName, gender: data.gender, email: data.email, phone: lookupPhone.trim(), sessionToken: freshToken });
      } else {
...
function Simulator({ module, candidate, onBack, onSessionInvalid }) {
  ...
  const savedThisSessionRef = useRef(false); // évite un double enregistrement

  // Charge les tentatives précédentes sur ce module, pour les afficher
  // avant que le candidat ne redémarre une simulation.
  useEffect(() => {
    if (!candidate?.phone) return;
    fetch(`/api/progress/history?phone=${encodeURIComponent(candidate.phone)}&moduleId=${encodeURIComponent(module.id)}`)
      .then(r => r.json())
      .then(data => setHistory(data.results || []))
      .catch(() => {});
  }, [candidate, module.id]);

  function saveSessionResult(averageScore, questionsCount, mode) {
    if (!candidate?.phone || savedThisSessionRef.current) return;
  ...
  const baseTextRef = useRef(""); // texte déjà présent avant de démarrer ce segment de dictée
  ...
  // Reconnaissance vocale : dicte la réponse du candidat par segments courts.
  // Choix délibéré plutôt qu'une session continue unique : le mode "continuous"
  // de Chrome est connu pour se couper silencieusement sur Android, surtout
  // avec une connexion mobile qui varie (la reconnaissance vocale du navigateur
  // envoie l'audio à un serveur en ligne). Des segments courts, où chaque fin
  // de phrase déclenche naturellement l'arrêt, sont beaucoup plus fiables —
  // le candidat appuie à nouveau pour continuer, le texte déjà dicté est
  // conservé et complété, jamais perdu.
  function startListening() {
    ...
    recognition.continuous = false; // un segment à la fois : plus court, plus fiable
    ...
    recognition.onresult = (event) => {
      ...
      setInput((base + sep + finalTranscript + interim).trim());
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === "no-speech") {
        return; // silence détecté, ce n'est pas une vraie erreur — pas de message
      }
      if (event.error === "not-allowed" || event.error === "permission-denied") {
        ...
        setMicError("Petite interruption du micro — appuie de nouveau pour continuer à dicter.");
      }
    };

    recognition.onend = () => {
      // Fin naturelle du segment (silence détecté ou phrase terminée) : le
      // bouton redevient disponible pour que le candidat poursuive d'un
      // simple appui, sans qu'aucun texte déjà dicté ne soit perdu.
      setListening(false);
      setListeningInterim("");
    };
  ...
  const start = async () => {
    ...
    const civility = candidate?.gender === "F" ? "Mme" : candidate?.gender === "M" ? "M." : "";
    const candidateName = candidate?.fullName?.trim();
    const greetingInstruction = candidateName
      ? `Le/la candidat(e) s'appelle ${civility ? `${civility} ` : ""}${candidateName}. Salue-le/la nommément dans ta toute première phrase (par exemple : "Bonjour ${civility ? `${civility} ` : ""}${candidateName}, bienvenue sur le simulateur d'entretien Héméra.") avant de te présenter et de poser la première question.\n\n`
      : "";
    const contextLine = (secteur.trim() || poste.trim())
      ...
      : "";
    const fullPrompt = greetingInstruction + contextLine + module.prompt;
    ...
    if (reply) {
      ...
      const greetingName = candidateName ? `${civility ? `${civility} ` : ""}${candidateName}` : "";
      setMessages([
        { role: "user", content: fullPrompt },
        {
          role: "assistant",
          content: `${greetingName ? `Bonjour ${greetingName}, bienvenue` : "Bienvenue"} sur le simulateur d'entretien Héméra. Je serai votre recruteur pour cette session — installez-vous confortablement, prenez le temps qu'il vous faut pour répondre, et faisons de cet échange une vraie préparation à la hauteur de vos ambitions.\n\n${sessionBankRef.current[0].q}`,
        },
      ]);
    }
  ...
  const send = async () => {
    ...
    if (basicMode) {
      ...
      function closeBasicInterview() {
        ...
        saveSessionResult(avgStars, basicScoreRef.current.count, "basic");
      ...
      if (passed) {
        ...
        if (attemptsRef.current >= 2) {
          ...
          // Réponse trop courte (un mot, une phrase évasive) : sanction immédiate
          // et visible, ET une vraie méthode à suivre — pas juste "développez".
          // On reste sur la même question, comme le ferait un recruteur exigeant.
          reply = `⭐☆☆☆☆\nUne réponse aussi brève ne convaincrait aucun recruteur. En entretien, chaque question est une occasion de vous vendre et de convaincre votre interlocuteur — pas de s'en tirer en une phrase.\n\n📋 Structurez votre réponse ainsi :\n1️⃣ Une affirmation claire et directe\n2️⃣ Un exemple concret et vécu\n3️⃣ Le résultat ou l'impact obtenu\n4️⃣ Le lien avec le poste que vous visez\n\nReprenez votre réponse à cette question :\n${current.q}`;
        } else {
    ...
    let updatedTotal = score.total;
    let updatedCount = score.count;
    if (starMatch) {
      const stars = starMatch[0].length;
      updatedTotal += stars;
      updatedCount += 1;
      setScore({ total: updatedTotal, count: updatedCount });
    }
    if (isFinalAnswer) {
      const globalNoteMatch = reply.match(/Note globale\s*:?\s*(\d+(?:[.,]\d+)?)\s*\/\s*5/i);
      const finalAvg = globalNoteMatch
        ? parseFloat(globalNoteMatch[1].replace(",", "."))
        : (updatedCount > 0 ? updatedTotal / updatedCount : null);
      if (finalAvg !== null) saveSessionResult(finalAvg, updatedCount, "ai");
    }
    setMessages(p => [...p, { role: "assistant", content: reply }]);
    setLoading(false);
  };
        ...
        <div style={{
          ...
          {history.length > 0 && (
            <div style={{
              ...
              background: T.orFond, border: `1px solid ${T.or}33`,
              ...
              <div style={{
                ...
                fontFamily: "'Space Mono', monospace",
              }}>Tes dernières tentatives sur ce module</div>
              {history.slice(0, 4).map((h, i) => (
                <div key={h.id} style={{
                  display: "flex", justifyContent: "space-between",
                  fontSize: "0.78rem", color: T.blanc, padding: "4px 0",
                  borderTop: i > 0 ? "1px solid #00000010" : "none",
                }}>
                  <span>{new Date(h.created_at).toLocaleDateString("fr-FR", { day: "numeric", month: "short" })}</span>
                  <span style={{ color: T.or, fontWeight: 700 }}>
                    {Number(h.average_score).toFixed(1)}/5 {"⭐".repeat(Math.round(h.average_score))}
                  </span>
                </div>
              ))}
              {history.length >= 2 && (() => {
                const diff = history[0].average_score - history[history.length - 1].average_score;
                if (Math.abs(diff) < 0.15) return null;
                return (
                  <div style={{ fontSize: "0.75rem", marginTop: 8, color: diff > 0 ? "#4A7B4A" : T.braise }}>
                    {diff > 0 ? "↗" : "↘"} {diff > 0 ? "Progression" : "Recul"} de {Math.abs(diff).toFixed(1)} point{Math.abs(diff) >= 2 ? "s" : ""} depuis ta première tentative
                  </div>
                );
              })()}
            </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {!listening && hasUsedMic && input.trim() && !micError && (
          <div style={{ fontSize: "0.7rem", color: T.gris }}>
            🎤 Appuie de nouveau sur le micro pour continuer à dicter
          </div>
        )}
        {micError && !listening && (
...
function Register({ onRegistered, onBack }) {
  ...
  function handleSubmit(e) {
    ...
    if (!gender) {
      setError("Merci d'indiquer ta civilité, pour un accueil personnalisé pendant l'entretien.");
      return;
    }
    ...
    setError("");
    onRegistered({ fullName: fullName.trim(), gender, email: email.trim(), phone: cleanPhone });
  }
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  background: gender === opt.value ? T.or : T.graphite,
                  border: `1px solid ${gender === opt.value ? T.or : "#E8D2AC"}`,
                  borderRadius: 4,
                  color: gender === opt.value ? "#fff" : T.blanc,
                  fontSize: "0.82rem", fontWeight: gender === opt.value ? 700 : 400,
                  cursor: "pointer", transition: "all 0.15s",
                }}
              >{opt.label}</button>
            ))}
          </div>
          <p style={{ color: "#A88B6F", fontSize: "0.68rem", marginTop: -10, marginBottom: 16 }}>
            Le recruteur virtuel t'appellera "M." ou "Mme" pendant l'entretien, pour une expérience plus naturelle.
          </p>
...
export default function App() {
  ...
  const handleRegistered = (info) => {
    ...
    fetch("/api/profile/register", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        phone: info.phone, email: info.email, fullName: info.fullName, gender: info.gender,
  ...
  // Reconnexion automatique via le lien reçu par email après approbation
  // d'un paiement (ex : https://.../?connect=077XXXXXX) — le candidat n'a
  // rien à ressaisir, comme une vraie connexion en un clic.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const connectPhone = params.get("connect");
    if (!connectPhone) return;

    const freshToken = generateSessionToken();
    fetch("/api/profile/lookup", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ phone: connectPhone, sessionToken: freshToken }),
    })
      .then(r => r.json())
      .then(data => {
        if (data.found) {
          const info = { fullName: data.fullName, gender: data.gender, email: data.email, phone: connectPhone, sessionToken: freshToken };
          localStorage.setItem("hemera_candidate", JSON.stringify(info));
          setCandidate(info);
          setPage("modules");
        }
      ...
      .catch(() => {})
      .finally(() => {
        // Nettoie l'URL pour ne pas laisser le numéro visible ni relancer
        // la connexion automatique à chaque rafraîchissement de page.
